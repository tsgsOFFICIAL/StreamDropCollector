...
namespace UI.Views
{
    public partial class HiddenWebViewHost : Window, IWebViewHost, IDisposable
    {
        ...
        /// <summary>
        /// Asynchronously captures the value of the "Client-Integrity" header from a successful GraphQL request to Twitch's GQL endpoint.
        /// </summary>
        /// <param name="timeoutMs"></param>
        /// <param name="ct"></param>
        /// <returns></returns>
        /// <exception cref="InvalidOperationException"></exception>
        public async Task<string> CaptureIntegrityFromSuccessfulGqlAsync(int timeoutMs = 15000, CancellationToken ct = default)
        {
            TaskCompletionSource<string> tcs = new TaskCompletionSource<string>(TaskCreationOptions.RunContinuationsAsynchronously);
            ConcurrentDictionary<string, string> pendingIntegrity = new ConcurrentDictionary<string, string>(); // requestId → Client-Integrity token
            ...
            await Dispatcher.InvokeAsync(async () =>
            {
                ...
                void OnRequest(object? s, CoreWebView2DevToolsProtocolEventReceivedEventArgs e)
                {
                    try
                    {
                        JsonElement json = JsonDocument.Parse(e.ParameterObjectAsJson).RootElement;
                        string url = json.GetProperty("request").GetProperty("url").GetString() ?? "";

                        if (!url.Contains("gql.twitch.tv/gql", StringComparison.OrdinalIgnoreCase))
                            return;

                        string requestId = json.GetProperty("requestId").GetString() ?? "";
                        JsonElement headers = json.GetProperty("request").GetProperty("headers");

                        string? token = null;

                        // Try exact match first
                        if (headers.TryGetProperty("Client-Integrity", out JsonElement exact))
                            token = exact.GetString();
                        else
                        {
                            // Case-insensitive fallback
                            foreach (JsonProperty prop in headers.EnumerateObject())
                            {
                                if (prop.Name.Equals("Client-Integrity", StringComparison.OrdinalIgnoreCase))
                                {
                        ...
                        if (!string.IsNullOrEmpty(token))
                            pendingIntegrity[requestId] = token!;
                    }
                    catch (Exception ex)
                    {
                        AppLogger.Warn("WebViewCapture", $"CaptureIntegrity OnRequest failed: {ex.Message}");
                    }
                }
                ...
                void OnResponse(object? s, CoreWebView2DevToolsProtocolEventReceivedEventArgs e)
                {
                    ...
                    {
                        JsonElement json = JsonDocument.Parse(e.ParameterObjectAsJson).RootElement;
                        string url = json.GetProperty("response").GetProperty("url").GetString() ?? "";

                        if (!url.Contains("gql.twitch.tv/gql", StringComparison.OrdinalIgnoreCase))
                            return;

                        int status = json.GetProperty("response").GetProperty("status").GetInt32();
                        string requestId = json.GetProperty("requestId").GetString() ?? "";

                        // Only resolve on a successful response that has a matching integrity token
                        if (status == 200 && pendingIntegrity.TryGetValue(requestId, out string? token))
                        {
                            Cleanup();
                    ...
                    {
                        AppLogger.Warn("WebViewCapture", $"CaptureIntegrity OnResponse failed: {ex.Message}");
                    }
                }

                void Cleanup()
                {
                    requestReceiver.DevToolsProtocolEventReceived -= OnRequest;
                    responseReceiver.DevToolsProtocolEventReceived -= OnResponse;
                }
            ...
            Task<string> winner = await Task.WhenAny(tcs.Task, timeoutTask).ConfigureAwait(false);

            string result = await winner;

            if (string.IsNullOrEmpty(result))
                throw new InvalidOperationException("CaptureIntegrityFromSuccessfulGqlAsync timed out");
